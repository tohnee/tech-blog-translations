...
**Note:** environments will be created in ROOT\_DIR/envs by default, you can use the `-p` instead of the `-n` flag in the conda commands above in order to specify a custom path.
...
As mentioned in the short introduction above (and in more detail in my separate [PCA article](https://sebastianraschka.com/Articles/2014_pca_step_by_step.html)), PCA is commonly used in the field of pattern classification for feature selection (or dimensionality reduction).  
...
[Stepping through a Linear Discriminant Analysis - using Python’s NumPy and matplotlib](https://github.com/rasbt/pattern_classification/blob/master/dimensionality_reduction/projection/linear_discriminant_analysis.ipynb).

Like we did in the PCA section above, we will use a `scikit-learn` funcion, [`sklearn.lda.LDA`](https://scikit-learn.org/stable/modules/generated/sklearn.discriminant_analysis.LinearDiscriminantAnalysis.html) in order to transform our training data onto 2 dimensional subspace:
