---
title: "UW-Madison Student Projects Gallery"
source: https://sebastianraschka.com/blog/2019/student-gallery-1.html
crawled: 2026-09-06
---

# UW-Madison Student Projects Gallery

Not too long ago, in the Summer of 2018, I was super excited to join the [Department of Statistics](https://stat.wisc.edu) at the University of Wisconsin-Madison after obtaining my Ph.D. after ~5 long and productive years. Now, two semesters later after finals’ week, I finally found some quiet days to look back on what’s happened since then.

During my first year at UW-Madison, besides moving my [research projects](http://pages.stat.wisc.edu/~sraschka/publications/) forward, my life was centered around the two courses I was teaching (~70 students each!). In fall, I started with designing and teaching a new [Machine Learning course](http://pages.stat.wisc.edu/~sraschka/teaching/stat479-fs2018/), and after a relatively short 2-week winter break, I did the same with a new [Deep Learning course](http://pages.stat.wisc.edu/~sraschka/teaching/stat479-ss2019/) this spring. Since these were new courses/topics in our department, I started from scratch concerning lecture material. I really like that, actually; however, it also was a lot of work! And in the end, it was very rewarding!
...
2. A ~10-min presentation at the end of the semester to practice communication skills as well as sharing interesting results and valuable experiences with the other students – in the spirit of a small conference.
3. A final ~8-page report [modeled after](https://github.com/rasbt/stat479-deep-learning-ss19/tree/master/report-template) a CVPR conference paper (mainly because it is a nice template) to practice written communication skills and gain practice concerning further research endeavors.

If you are interested and/or find it useful, all my lecture material is available on GitHub:

- [Fall 2018 Machine Learning course](https://github.com/rasbt/stat479-machine-learning-fs18)
- [Spring 2019 Deep Learning course](https://github.com/rasbt/stat479-deep-learning-ss19)
...
- [GitHub repository](https://github.com/nhaselow/stat479)
- [Report PDF](https://github.com/nhaselow/stat479/blob/master/Audio%20Classification%20Using%20Convolutional%20Neural%20Networks.pdf)

In this project, Poet Larsen, Reng Chiz Der, and Noah Haselow were exploring modern approaches for audio classification. In particular, Poet, Reng, and Noah used deep learning architectures that were originally developed for computer vision, namely convolutional neural networks (CNNs). The students implemented CNN architectures to capture temporal relationships in audio clips from spectrograms. They found that a [Gated-CNN architecture](https://arxiv.org/abs/1612.08083) was particularly helpful for mitigating vanishing gradient effects.
...
As a [benchmark](https://sebastianraschka.com/glossary/#benchmark "Benchmark") dataset for this study, the students used [Google’s Voice Ontology AudioSet](https://research.google.com/audioset/ontology/index.html). The dataset was trimmed to 5000 audio clips and 10 classes due to computational processing constraints (the students observed that downloading all 2 million videos and convert them to audio clips would take approximately 3-5 months). After obtaining the 5000 clips, the students turned the audio samples into spectrograms via a discrete short-time Fourier transform. Two examples are shown below:

![Student gallery 1 spectogram](https://sebastianraschka.com/images/blog/2019/student-gallery-1/spectogram.webp)

Primarily, the students focused on a Gated-CNN architecture, inspired by Dauphin *and others* ([“Language modeling with gated convolutional networks”](https://arxiv.org/abs/1612.08083)) and Xu *and others* ([“Large-scale weakly supervised audio classification using gated convolutional neural network”](https://arxiv.org/abs/1710.00343)):
...
Furthermore, the students compared the Gated-CNN approach with other CNN architectures developed for image classification, such as [VGG](https://arxiv.org/abs/1409.1556), [AlexNet](http://papers.nips.cc/paper/4824-imagenet-classification-with-deep-convolutional-neural-networ), and [ResNet](http://openaccess.thecvf.com/content_cvpr_2016/html/He_Deep_Residual_Learning_CVPR_2016_paper.html). The outcome is summarized in the two tables below:
...
- [PDF report](https://github.com/sberglin/Projects-and-Papers/blob/master/3D%20CNN/Report.pdf)
...
Since this study was performed for educational purposes, Sam Berglin, Jiahui Jian, and Lian Zheming used a [3D MNIST dataset](https://www.kaggle.com/daavoo/3d-mnist) to compare a “traditional” CNN architecture, an all-convolutional CNN architecture, and an architecture that the students refer to as “Connectome” – an architecture that was [designed for fMRI data](https://link.springer.com/chapter/10.1007%2F978-3-030-00889-5_16) (traditionally, the term connectome refers to the wiring diagram of a biological nervous system). The 3D MNIST dataset consists of 3D point clouds generated from the original MNIST digits. Each 3D image contains 4096 pixels (16x16x16 pixels) and has 3 color channels obtained from voxelization (3x4096 = 12,288 pixels per image in total):
...
Then, inspired by Springenberg *and other*’s “[All Convolutional Net](https://arxiv.org/abs/1412.6806),” the students replaced all max-pooling and fully connected layers by convolutional layers; however, the predictive performance remained essentially unchanged:
...
As you may know, [it’s relatively easy to achieve >90% test accuracy on the traditional 2D MNIST dataset with a generalized linear model such as multinomial logistic regression](https://github.com/rasbt/deep-learning-book/blob/master/code/model_zoo/pytorch_ipynb/softmax-regression.ipynb). Keeping that in mind, the performance of the CNN-based models is actually relatively good. Also, note that besides the challenge of having another dimension (16x more features), the number of training examples in the 3D MNIST dataset is also 5-times smaller compared to the original 2D MNIST dataset, which makes it easier to overfit.

## Judging a Book by its Cover: A Modern Approach

- [GitHub repository](https://github.com/yienxu/judging-a-book-by-its-cover)
- [Report PDF](https://github.com/yienxu/judging-a-book-by-its-cover/blob/master/Report.pdf)
...
2. using “model interpretation” algorithms to explain the predictions – which cover features contribute most towards predicting whether books are categorized as popular or unpopular;
3. using a Generative Adversarial Nets (GAN) model to generate new popular book covers.
...
The dataset that the students used was [Kaggle’s Goodreads’ Best Books Ever](https://www.kaggle.com/meetnaren/goodreads-best-books), which contained 53,618 book covers of various sizes (most of them are in RGB format). The labels were scores computed by Goodreads based on multiple factors, including user average ratings and review scores. The scores were given on a ranking scale, and to simplify the modeling problem, the students divided the ranking scale into five classes (from best to worst). Note that these class labels are on an ordinal (as opposed to a nominal scale). Hence, for the first subtask, predicting the popularity of book covers, the students used the [CORAL framework](http://arxiv.org/abs/1901.07884) for ordinal regression with CNNs that I recently developed with Wenzhi Cao and Vahid Mirjalili as opposed to a traditional Softmax+Cross-Entropy classifier approach.
...
- [GitHub repository](https://github.com/LingfengZhu/Face-to-painting-machine)
- [Report PDF](https://github.com/LingfengZhu/Face-to-painting-machine/blob/master/STAT479_Project_Report.pdf)
- [Presentation slides](https://github.com/LingfengZhu/Face-to-painting-machine/blob/master/479_Presentation.pdf)
...
2. performing face matching with triplet loss;
3. using a deep-learning approach for photographic style transfer to apply various artistic styles to face images.
...
For training the convolutional neural network models for face matching based on FaceNet’s triplet loss function, the students used the [Labeled Face in the Wild](http://vis-www.cs.umass.edu/lfw/) (LFW) dataset. This dataset contained more than 13,000 face images collected from the internet, consisting of photographs of ~6000 different persons.

As suggested by the original FaceNet paper ([Schroff and others, 2015](http://arxiv.org/abs/1503.03832)), the students found that training on all positive-anchor and negative-anchor examples were not ideal (slow convergence).
...
- [GitHub repository v1](https://github.com/c-lynn/stat479-genre-classification)
- [Report PDF v1](https://github.com/c-lynn/stat479-genre-classification/blob/master/Lyrics_Final_Report.pdf)
- [GitHub repository v2](https://github.com/jwang862/Music-Genre-Classification-Based-on-Lyrics)
- [Report PDF v2](https://github.com/jwang862/Music-Genre-Classification-Based-on-Lyrics/blob/master/Final%20Report.pdf)
...
## Projects from STAT479: Machine Learning
...
- Authors: Samuel Ilic, Meenmo Kang, Jonathan Santoso
- [GitHub repository](https://github.com/meenmo/Forecasting_Stock_Returns_via_Supervised_Learning)
