...
- for one or more epochs:
  - for each weight \(j\)
    - \(w\_j := w + \Delta w\_j\), where: \(\Delta w\_j= \eta \sum\_i (\text{target}^{(i)} - \text{output}^{(i)})x\_{j}^{(i)}\)
...
- for one or more epochs, or until approx. cost minimum is reached:
  - for training sample \(i\):
    - for each weight \(j\)
      - \(w\_j := w + \Delta w\_j\), where: \(\Delta w\_j= \eta (\text{target}^{(i)} - \text{output}^{(i)})x\_{j}^{(i)}\)
