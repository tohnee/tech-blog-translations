...
The leading PEFT method is low-rank adaptation, or LoRA. LoRA replaces each weight matrix W from the original model with a modified version W′=W+γBAW' = W + \gamma BAW′=W+γBA, where B and A are matrices that together have far fewer parameters than W, and γ\gammaγ is a constant scaling factor. In effect, LoRA creates a low-dimensional representation of the updates imparted by fine-tuning.
...
For these experiments, we used a basic policy gradient algorithm with an importance sampling correction; objective=∑tplearnerpsamplerAdvt\text{objective} =\sum_t \frac{p_{\text{learner}}}{p_{\text{sampler}}} Adv_tobjective=∑t​psampler​plearner​​Advt​.See [Your Efficient RL Framework Secretly Brings You Off-Policy RL Training](https://fengyao.notion.site/off-policy-rl)
We used a GRPO-like centering scheme[DeepSeekMath: Pushing the Limits of Mathematical Reasoning in Open Language Models](https://arxiv.org/abs/2402.03300) (Shao et al, 2024) where we sample multiple completions per problem and subtract the mean reward per group.
...
We can use more precise numbers based on our experiments. In the MATH example, we trained on ~10,000 problems with 32 samples per problem. Assuming each completion yields a single bit of information, the whole training process only needs to absorb 320,000 bits. Rank-1 LoRA for Llama-3.1-8B already has 3M parametersWe calculated this by adding up rank·dind_{in}din​ (for matrix A) and rank·doutd_{out}dout​ (for B) over all weight matrices in the model., almost 10 times that number. Even at rank-1, LoRA has more than enough capacity to absorb all the information provided during training.
...
W′=W+αrBAW' = W + \frac{\alpha}{r}BAW′=W+rα​BA

Where rrr is the LoRA rank, α\alphaα is the LoRA scaling factor, and AAA, BBB are the LoRA weight matrices (of rank rrr). We use α=32\alpha = 32α=32 for the experiments in this article, following standard practice from other implementations.

The 1/r1/r1/r scaling factor makes the optimal learning rate approximately independent of rank. In fact, a stronger condition holds – the learning curve is exactly the same at the beginning of training, regardless of rank. This effect is striking, and in our experiments the closeness of the learning curves for different ranks had us worried that a bug caused the rank parameter to be ignored. It follows that in a short training regime, the optimal LR is also independent of rank. However, as we showed above in our plots of learning rate vs loss (Figure 2), optimal LR has some rank-dependence in the longer-training regime.
...
We can partly explain this result by looking at the expected update to the LoRA matrix after the very first training update. We can think of the LoRA product BABABA as the sum of rrr rank-1 outer products: BA=∑i=1rbiaiT=∑i=1rΔiBA = \sum_{i=1}^r b_i a_i^T = \sum_{i=1}^r \Delta_iBA=∑i=1r​bi​aiT​=∑i=1r​Δi​, where we define Δi=biaiT\Delta_i = b_i a_i^TΔi​=bi​aiT​. Here, ∂Loss/∂Δi\partial \text{Loss}/\partial \Delta_i∂Loss/∂Δi​ is the same for all iii; however the gradients ∂Loss/∂bi\partial \text{Loss}/\partial b_i∂Loss/∂bi​ and ∂Loss/∂ai\partial \text{Loss}/\partial a_i∂Loss/∂ai​ will depend on the initialization (∂Loss/∂bi\partial \text{Loss}/\partial b_i∂Loss/∂bi​ depends on aia_iai​, for example). Since the initialization of aia_iai​ and bib_ibi​ do not depend on rank, it follows that E[Δi]\mathbb{E}[\Delta_i]E[Δi​] is the same for all iii and does not depend on rank. At the first step of training, the expected update from each of these terms is equal and independent of the rank. It follows that (1/r)∑i=1rΔi(1/r)\sum_{i=1}^r \Delta_i(1/r)∑i=1r​Δi​ is just a sample average of rrr terms with the same expectation, so the expectation of the average, i.e., the change to the adapter (1/r)BA(1/r)BA(1/r)BA, doesn’t depend on the rank.

### Parametrization invariances

There are four hyperparameters potentially applicable to LoRA:

1. The scale factor ααα which appears in α/rα / rα/r.
2. The learning rate for the down-projection matrix AAA, LRALR_ALRA​
3. The learning rate for the up-projection matrix BBB, LRBLR_BLRB​.
4. The initialization scale of matrix AAA, initA\text{init}_AinitA​. For a random initialization, this is the standard deviation of AAA‘s initial elements. Matrix BBB is initialized to zero, so there is no need to define initB\text{init}_BinitB​.

Having to tune four different parameters may seem overwhelming. However, invariances in the training dynamics mean that two of these are redundant, and learning behavior is determined by two. We show this invariance by noting that when training with Adam and ε=0ε = 0ε=0, theWe can extend this result to ε>0ε > 0ε>0; we need to scale it by 1/q1/q1/q because the gradients are scaled by that factor. optimization process is invariant to the following two-parameter transformation.

For p,q>0p, q > 0p,q>0:

- α→1pq⋅αα → \frac{1}{pq} \cdot αα→pq1​⋅α
- initA→p⋅initA\text{init}_A → p \cdot \text{init}_AinitA​→p⋅initA​
- LRA→p⋅LRALR_A → p \cdot LR_ALRA​→p⋅LRA​
- LRB→q⋅LRBLR_B → q \cdot LR_BLRB​→q⋅LRB​
...
- α⋅initA⋅LRBα \cdot \text{init}_A \cdot LR_Bα⋅initA​⋅LRB​. This determines the scale of initial updates, or, equivalently, the initial slope of the learning curve. Since BBB is initialized to zero, LRALR_ALRA​ and the initial updates to AAA are irrelevant.
- initA/LRA\text{init}_A / LR_AinitA​/LRA​. Since Adam updates the elements of AAA by approximately LRALR_ALRA​ at each step, this timescale parameter determines the number of steps it takes to significantly transform AAA away from its initial state.

We can reinterpret some proposals from previous work on LoRA in terms of this basis.

LoRA+[LoRA+: Efficient Low Rank Adaptation of Large Models](https://arxiv.org/abs/2402.12354) (Hayou et al, 2024) proposes to use different LRs on AAA and BBB, with a higher rate for BBB. Expressed in terms of our basis above, increasing LRBLR_BLRB​ is equivalent to increasing initA/LRA\text{init}_A/LR_AinitA​/LRA​ so that AAA changes on a longer timescale.

[Unsloth’s LoRA Hyperparameter Guide](https://docs.unsloth.ai/get-started/fine-tuning-llms-guide/lora-hyperparameters-guide) recommends using higher values of ααα for high-rank LoRA, e.g. by avoiding the 1/r1/r1/r scaling. This is also equivalent to increasing initA/LRA\text{init}_A/LR_AinitA​/LRA​. When we increase ααα, LRALR_ALRA​ and LRBLR_BLRB​ need to be lowered in compensation to get the same update size. This in turn simply makes LRALR_ALRA​ smaller relative to initA\text{init}_AinitA​.

In our experiments, we used the standard parametrization used in the Huggingface `peft` library[PEFT: State-of-the-art Parameter-Efficient Fine-Tuning methods](https://github.com/huggingface/peft) (Mangrulkar et al, 2022) proposed by Hu et al: a uniform distribution for AAA with scale 1/din1/\sqrt{d_{in}}1/din​​, zero initialization for BBB, the same LR for both, and α=32α = 32α=32. We were unable to improve on these hyperparameters in our experimentation.
...
We don’t yet have an adequate theoretical explanation for this observation. We can attempt to derive this result from the facts that optimal LoRA LR is invariant to rank and that full-rank LoRA is directly comparable to FullFT. This analysis suggests a LR ratio of the model’s hidden size divided by 2⋅α2 \cdot \alpha2⋅α, which doesn’t match the empirical result of the optimal ratio being fixed at 10 independent of the base model.
...
LR=MLoRA⋅(2000hidden size)model pow+LoRA pow\text{LR} = M_{\text{LoRA}} \cdot \left(\frac{2000}{\text{hidden size}}\right)^{\text{model pow} + \text{LoRA pow}}LR=MLoRA​⋅(hidden size2000​)model pow+LoRA pow

Where:

- MLoRAM_{\text{LoRA}}MLoRA​ is a multiplier applied when LoRA is used (1 if FullFT)
- model pow\text{model pow}model pow is an exponent adjustment, calculated separately for each model source (Llama and Qwen)
- LoRA pow\text{LoRA pow}LoRA pow is an additional exponent adjustment for LoRA
- hidden size\text{hidden size}hidden size is the dimension of the residual stream of the model.

We scored a predicted learning rate by using linear interpolation to predict the loss, based on the data from our sweep, and rated the parameters by summing the predicted loss over the 14 problems. Our optimization found a multiplier of 9.8 for LoRA over FullFT, and different dependence on hidden_size for Qwen3 and Llama models, but LoRA LRs had the same dependence on hidden_size as FullFT LRs, i.e., the optimization found LoRA pow=0\text{LoRA pow} = 0LoRA pow=0.
...
We can sharpen the bits-counting argument in information-theoretic terms. Consider an episode, consisting of a trajectory τ\tauτ and final reward, as a message (i.e., a noisy channel) that provides some information about the unknown reward function RRR. We’ll condition on the current policy and training history and look at the mutual information between the policy gradient estimator and RRR. The REINFORCE update is G=S⋅AdvG = S \cdot \text{Adv}G=S⋅Adv with S=∇log⁡pθ(τ)S = \nabla \log p_\theta(\tau)S=∇logpθ​(τ). SSS is independent of RRR given the history, so the only RRR-dependent component is the scalar advantage.

By the data processing inequality:

I(G;R∣history)≤I((S,Adv);R∣history)=I(Adv;R∣S,history)≤H(Adv).I(G ; R | \text{history}) \leq I((S, \text{Adv}) ; R | \text{history}) = I(\text{Adv} ; R | S, \text{history}) \leq H(\text{Adv}).I(G;R∣history)≤I((S,Adv);R∣history)=I(Adv;R∣S,history)≤H(Adv).

If we quantize the advantage into BBB bins, then H(Adv)≲log⁡(B)H(\text{Adv}) \lesssim \log(B)H(Adv)≲log(B). That is, the number of bits of useful information gleaned per episode is O(1)O(1)O(1), independent of model size. These bits tell us which member of a discrete set of reward functions (or, equivalently, optimal-policy classes) we’re in. This analysis of mutual information mirrors what’s used in some theoretical analysis of optimization algorithms.[Information Complexity of Black-Box Convex Optimization: A New Look via Feedback Information Theory](https://www.mit.edu/~rakhlin/papers/ibc_optimization.pdf) (Raginsky and Rakhlin, 2009) Note that this estimate is an *upper* bound on the information absorbed by training; the actual amount learned will depend on the policy initialization and other details. For example, if we initialize with a policy that doesn’t get any reward, then the entropy of the advantage is zero (not log(B)), and it won’t learn anything.
...
- W∈RN×NW \in \mathbb{R}^{N \times N}W∈RN×N is a weight matrix
- x∈RNx \in \mathbb{R}^Nx∈RN is an input vector
- y=Wx∈RNy = Wx \in \mathbb{R}^Ny=Wx∈RN is an output vector
- xˉ,yˉ∈RN\bar{x}, \bar{y} \in \mathbb{R}^Nxˉ,yˉ​∈RN are the gradients of the loss with respect to xxx and yyy, computed in the backward pass
- Wˉ∈RN×N\bar{W} \in \mathbb{R}^{N \times N}Wˉ∈RN×N is the gradient of the loss with respect to WWW

Full fine-tuning performs the following operations:

**Forward**

1. y=Wxy = Wxy=Wx (N2N^2N2 multiply–adds)

**Backward**

2. xˉ=WTyˉ\bar{x} = W^T \bar{y}xˉ=WTyˉ​ (N2N^2N2 multiply–adds)
3. Wˉ+=xyˉT\bar{W} \mathrel{+}= x \bar{y}^TWˉ+=xyˉ​T (N2N^2N2 multiply–adds)

The forward pass requires N2N^2N2 multiply-adds, and the backward pass requires another 2⋅N22 \cdot N^22⋅N2 for 3N23N^23N2 total. Training, which requires both, thus uses 3 times the FLOPs of forward-only inference.

With LoRA, we replace WWW by W+BAW + BAW+BA, where B∈RN×RB \in \mathbb{R}^{N \times R}B∈RN×R and A∈RR×NA \in \mathbb{R}^{R \times N}A∈RR×N, with R≪NR \ll NR≪N. Since we only update Aˉ\bar{A}Aˉ and Bˉ\bar{B}Bˉ, we replace the third step of updating Wˉ\bar{W}Wˉ with a much cheaper operation. AAA and BBB are N⋅RN \cdot RN⋅R matrices, so the full forward-backward computation on each requires 3NR3NR3NR multiply-adds instead of 3N23N^23N2 for WWW. The total for both is 6NR6NR6NR. We also perform the forward-backward pass on WxWxWx and xˉ\bar{x}xˉ, equivalent to the first two steps of FullFT. The total number of multiply-adds is 2N2+6NR2N^2 + 6NR2N2+6NR. With R≪NR \ll NR≪N, this is slightly more than 23\frac{2}{3}32​ of 3N23N^23N2.
