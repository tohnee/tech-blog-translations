...
  
Alibaba Group
††thanks: Authors are ordered alphabetically by the first name.
...
To foster community innovation and accessibility, we have made the Qwen2 model weights openly available on Hugging Face11
1
<https://huggingface.co/Qwen> and ModelScope22
2
<https://modelscope.cn/organization/qwen>, and the supplementary materials including example code on GitHub33
3
...
Detailed KV head configurations for various model sizes are reported in Section [2.2.3](#S2.SS2.SSS3 "2.2.3 Model Configuration ‣ 2.2 Model Architecture ‣ 2 Tokenizer & Model ‣ Qwen2 Technical Report").
...
As a substitute for the original FFN, the MoE FFN consists of nn individual FFNs, each serving as an expert.
Each token is directed to a specific expert EiE_{i} for computation based on probabilities assigned by a gated network GG:

|  |  |  |  |  |
| --- | --- | --- | --- | --- |
|  | 𝐩\displaystyle\mathbf{p} | =softmax⁡(G⁡(𝐱)),\displaystyle=\mathrm{softmax}\left(G\left(\mathbf{x}\right)\right), |  | (1) |
|  |  |  |  |  |
| --- | --- | --- | --- | --- |
|  | 𝐲\displaystyle\mathbf{y} | =∑i∈topk​(p)𝐩i​Ei​(𝐱).\displaystyle=\sum\nolimits_{i\in\text{top}_{k}\left({\textbf{p}}\right)}\mathbf{p}_{i}E_{i}(\mathbf{x}). |  | (2) |
...
Given the designated expert intermediate size hEh_{\text{E}}, the number of experts nn, and the original FFN intermediate size hFFNh_{\text{FFN}}, the FFN is replicated ⌈n×hE/hFFN⌉\left\lceil\nicefrac{{n\times h_{\text{E}}}}{{h_{\text{FFN}}}}\right\rceil times.
...
Table [1](#S2.T1 "Table 1 ‣ 2.2.2 Qwen2 Mixture-of-experts Model ‣ 2.2 Model Architecture ‣ 2 Tokenizer & Model ‣ Qwen2 Technical Report") lists the hyper-parameters and important information, e.g., the number of pre-trained tokens. Particularly, Qwen2-57B-A14B is upscaled from Qwen2-7B.
...
The post-training data primarily consists of two components: demonstration data 𝒟={(xi,yi)}\mathcal{D}=\{(x_{i},y_{i})\} and preference data 𝒫={(xi,yi+,yi−)}\mathcal{P}=\{(x_{i},y_{i}^{+},y_{i}^{-})\}, where xix_{i} represents the instruction, yiy_{i} represents a satisfactory response, and yi+y_{i}^{+} and yi−y_{i}^{-} are two responses to xix_{i}, with yi+y_{i}^{+} being the preferred choice over yi−y_{i}^{-}.
The set 𝒟\mathcal{D} is utilized in SFT, whereas 𝒫\mathcal{P} is employed in RLHF.
...
Through human annotation, we obtain the target response yiy_{i} and their positive and negative counterparts (yi+,yi−)(y_{i}^{+},y_{i}^{-}).
...
To optimize learning, the learning rate was gradually decreased from 7×10−67\times 10^{-6} to 7×10−77\times 10^{-7}.
...
In the offline training stage, we use a pre-compiled preference dataset 𝒫\mathcal{P} to maximize the difference in likelihood between yi+y_{i}^{+} and yi−y_{i}^{-} with Direct Preference Optimization (DPO, [Rafailov et al., 2023](#bib.bib59)).
...
The results are reported in Table [2](#S5.T2 "Table 2 ‣ 5.1.1 Core Capabilities ‣ 5.1 Base Language Models ‣ 5 Evaluation ‣ Qwen2 Technical Report").
...
The results are shown in Table [3](#S5.T3 "Table 3 ‣ Benchmarks and Evaluation Protocol ‣ 5.1.1 Core Capabilities ‣ 5.1 Base Language Models ‣ 5 Evaluation ‣ Qwen2 Technical Report").
...
The results can be found in Table [4](#S5.T4 "Table 4 ‣ Qwen2-57B-A14B ‣ 5.1.1 Core Capabilities ‣ 5.1 Base Language Models ‣ 5 Evaluation ‣ Qwen2 Technical Report").
...
The results are given in Table [5](#S5.T5 "Table 5 ‣ Qwen2-7B ‣ 5.1.1 Core Capabilities ‣ 5.1 Base Language Models ‣ 5 Evaluation ‣ Qwen2 Technical Report").
...
The results are presented in Table [6](#S5.T6 "Table 6 ‣ 5.2.1 Open Benchmark Evaluation ‣ 5.2 Instruction-tuned Model ‣ 5 Evaluation ‣ Qwen2 Technical Report").
...
The results are provided in Table [7](#S5.T7 "Table 7 ‣ Qwen2-72B-Instruct ‣ 5.2.1 Open Benchmark Evaluation ‣ 5.2 Instruction-tuned Model ‣ 5 Evaluation ‣ Qwen2 Technical Report").
...
The results can be found in Table [8](#S5.T8 "Table 8 ‣ Qwen2-57B-A14B-Instruct ‣ 5.2.1 Open Benchmark Evaluation ‣ 5.2 Instruction-tuned Model ‣ 5 Evaluation ‣ Qwen2 Technical Report").
...
As detailed in Table [9](#S5.T9 "Table 9 ‣ Qwen2-7B-Instruct ‣ 5.2.1 Open Benchmark Evaluation ‣ 5.2 Instruction-tuned Model ‣ 5 Evaluation ‣ Qwen2 Technical Report"), the Qwen2 models demonstrate a marked advantage over their predecessors in both core capabilities and instruction-following tasks.
...
The results are gathered in Table [10](#S5.T10 "Table 10 ‣ 5.2.2 In-house Automatic Evaluation ‣ 5.2 Instruction-tuned Model ‣ 5 Evaluation ‣ Qwen2 Technical Report") and Table [11](#S5.T11 "Table 11 ‣ 5.2.2 In-house Automatic Evaluation ‣ 5.2 Instruction-tuned Model ‣ 5 Evaluation ‣ Qwen2 Technical Report"), respectively.
...
As illustrated in Figure [1](#S5.F1 "Figure 1 ‣ Needle in a Haystack ‣ 5.2.3 Long Context Capabilities ‣ 5.2 Instruction-tuned Model ‣ 5 Evaluation ‣ Qwen2 Technical Report"), Qwen2-72B-Instruct exhibits exceptional accuracy in retrieving information from the entire 128K context.
...
Table [12](#S5.T12 "Table 12 ‣ Needle in a Haystack ‣ 5.2.3 Long Context Capabilities ‣ 5.2 Instruction-tuned Model ‣ 5 Evaluation ‣ Qwen2 Technical Report") reveals that the integration of YARN and DCA ([An et al., 2024](#bib.bib4)) notably improves Qwen2 models’ long-context abilities.
...
As shown in Table [12](#S5.T12 "Table 12 ‣ Needle in a Haystack ‣ 5.2.3 Long Context Capabilities ‣ 5.2 Instruction-tuned Model ‣ 5 Evaluation ‣ Qwen2 Technical Report"), integrating YARN and DCA substantially bolsters the long-context competencies of Qwen2 models on LV-Eval.
...
From Table [13](#S5.T13 "Table 13 ‣ 5.2.4 Multilingual Evaluation ‣ 5.2 Instruction-tuned Model ‣ 5 Evaluation ‣ Qwen2 Technical Report"), it can be found that on average Qwen2-72B-Instruct significantly outperforms GPT-3.5-Turbo and it is competitive with GPT-4-Turbo and slightly falls behind Claude-3-Opus.
...
The results are presented in Table [14](#S5.T14 "Table 14 ‣ 5.2.5 Safety & Responsibility ‣ 5.2 Instruction-tuned Model ‣ 5 Evaluation ‣ Qwen2 Technical Report"), where the proportion of harmful responses generated by the models are shown and the lower, the better.
...
| Contamination | Original | Non-Contam. | Δ\Delta | Original | Non-Contam. | Δ\Delta |
...
For a training sequence 𝐬t\mathbf{s}_{t}, we remove it if there is a test sequence 𝐬e\mathbf{s}_{e} such that |LCS​(𝐬t,𝐬e)|≥13|\text{LCS}(\mathbf{s}_{t},\mathbf{s}_{e})|\geq 13 and |LCS​(𝐬t,𝐬e)|≥0.6×min⁡(|𝐬t|,|𝐬e|)|\text{LCS}(\mathbf{s}_{t},\mathbf{s}_{e})|\geq 0.6\times\min(|\mathbf{s}_{t}|,|\mathbf{s}_{e}|).
