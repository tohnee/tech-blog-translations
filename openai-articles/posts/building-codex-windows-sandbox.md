...
2. The `sandbox-write` SID was granted write, execute, and delete access toThe current working directoryAny additional `writable_roots` configured in `config.toml`.
3. The sandbox setup explicitly denied that same SID write access to “read-only within writable” locations such as:`<cwd>/.git``<cwd>/.codex``<cwd>/.agents`
...
- `codex.exe` runs as the real Windows user. Then, in a sequence, Codex:Calls `LogonUserW(...)` for the sandbox user.Calls `CreateRestrictedToken(...)` on that sandbox-user token.Using that restricted sandbox-user token, calls `CreateProcessAsUserW(...)` to launch the final child.
