...
[#multimodal](https://vllm.ai/blog/tags/multimodal)[#rlhf](https://vllm.ai/blog/tags/rlhf)[#ecosystem](https://vllm.ai/blog/tags/ecosystem)
...
| --- | --- | --- |
| A wooden trail marker in a dense forest with **"Hidden Trail"** carved into the wood, surrounded by moss and foliage. | Hidden Trail — step 0  Hidden Trail — step 0 | Hidden Trail — step 120  Hidden Trail — step 120 |
| A birthday card interior with **"Make A Wish"** in cursive handwriting, surrounded by sparkling candles and colorful confetti. | Make A Wish — step 0  Make A Wish — step 0 | Make A Wish — step 120  Make A Wish — step 120 |
...
|  |  |
| --- | --- |
| Validation reward rising from 0.7 to 0.95  Validation reward rising from 0.7 to 0.95    validation reward increases stably | Rollout reward mean rising from ~0.15 to ~0.9  Rollout reward mean rising from ~0.15 to ~0.9    rollout reward mean increases (low start expected for non-CFG rollout) |
| critic/rewards/zero_std_ratio rising only after reward saturates  critic/rewards/zero\_std\_ratio rising only after reward saturates    zero-std ratio climbs only after reward saturates | actor/pg_clipfrac staying in healthy range  actor/pg\_clipfrac staying in healthy range    clip ratio stays in healthy range |
