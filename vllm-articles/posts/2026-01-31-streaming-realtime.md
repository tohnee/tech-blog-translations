...
[#multimodal](https://vllm.ai/blog/tags/multimodal)
...
*Note*: If you wish to know how to use the new streaming input or realtime API with vLLM, please refer to the following references:
...
*Note*: Even for traditional applications, where the full input needs to be read in order to generate the first output token, the ability to stream input as it becomes available can still be beneficial.
...
Let $X=(x_{0},x_{1},...,x_{T})$ denote the input sequence and $Y=(y_{0},y_{1},...,y_{T′})$ the output sequence. In streaming applications, the model should generate the output $y_{t}$ corresponding to input $x_{t}$ at time step $t$, with as little latency as possible. Concretely, one can think of $y_{t}$ as the transcription of an audio frame $x_{t}$ that is streamed into the model at time $t$.
...
This formulation is unsuitable for streaming, because generating $y_{i}$ requires processing the full input sequence $X$, which is not available in real time.

Instead, a streaming model must be able to predict $y_{i}$ using only past inputs and, optionally, a small amount of future context:

$P(y_{i}∣y_{i-1},...,y_{0},x_{i+δ},...,x_{i},...,x_{0}),$

where $δ$ is a lookahead parameter that should be as small as possible. In theory, $δ$ could be set to zero; in practice, a small delay is usually necessary to achieve reasonable performance.

As a result, training a streaming model requires:

- **i)** aligning input and output sequences such that $T^{′}=T$, and each $y_{i}$ is the correct output corresponding to $x_{i}$;
- **ii)** using an architecture that can process new inputs $x_{i+1}$ while previous inputs $x_{i},...,x_{0}$ have already been processed.

An intuitive architecture, as pioneered by [Delayed Streams Modeling](https://arxiv.org/pdf/2509.08753) and picked up by [Voxtral-Realtime](https://vllm.ai/blog/TODO), sum-pools input embeddings (e.g., speech embeddings) and output embeddings (e.g., text embeddings) into a single sequence of embeddings. The model then predicts
...
You can wait until the output corresponding to the last input has completed before sending the next input, but it's not required (input chunks are queued internally). Termination of the input stream is indicated by exiting from the async input generator or closing it using the `aclose` function. The returned outputs generator won't complete until all received inputs have been processed *and* the input generator has completed.
...
- [examples/online\_serving/openai\_realtime\_client.py](https://docs.vllm.ai/en/latest/examples/online_serving/openai_realtime_client/?h=realtime#openai-realtime-client): Basic WebSocket client
- [examples/online\_serving/openai\_realtime\_microphone\_client.py](https://docs.vllm.ai/en/latest/examples/online_serving/openai_realtime_microphone_client/#openai-realtime-microphone-client): Microphone integration
