...
RL workloads require executing the same policy twice to generate rollouts (rollout engine) and train the policy (trainer). The rollout engine samples a token under policy $μ$; the trainer later recomputes its log probability under policy $π$ using the same model parameters. Under synchronous RL, typical on-policy training assumes $μ=π$ (no train–inference mismatch). From a systems perspective, true on-policy training is hard due to floating-point non-associativity:
...
Across 50 steps, the mean pre-update rollout-versus-training absolute logprob difference reduced from $1.648\times10^{-2}$ to $6.744\times10^{-7}$, its standard deviation reduced from $4.035\times10^{-2}$ to $6.821\times10^{-7}$, and the average per-step maximum reduced from $5.073$ to $7.358\times10^{-6}$.
