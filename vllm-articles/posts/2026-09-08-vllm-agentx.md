...
[#agentic](https://vllm.ai/blog/tags/agentic)[#kv\_cache](https://vllm.ai/blog/tags/kv_cache)[#parallelism](https://vllm.ai/blog/tags/parallelism)[#large-scale-serving](https://vllm.ai/blog/tags/large-scale-serving)[#disaggregation](https://vllm.ai/blog/tags/disaggregation)[#performance](https://vllm.ai/blog/tags/performance)
...
![Figure 1: vLLM on SemiAnalysis AgentX. Total tokens per $1 of TCO against P90 interactivity for the best vLLM configuration of DeepSeek V4 Pro, MiniMax M3, and Kimi K3, with DeepSeek V4 Pro on GB300 NVL72 as a case study. Data source: SemiAnalysis AgentX.](https://vllm.ai/blog-assets/figures/2026-09-08-vllm-agentx/agentx-pareto-summary.png)
...
[Interactive figure](https://vllm.ai/blog-assets/interactive\_pages/vllm-agentx-pareto.html)
...
[Interactive figure](https://vllm.ai/blog-assets/interactive\_pages/agentic-workload-explorer.html)
...
![Figure 3: Full-stack optimization for agentic serving. The data plane manages a distributed shared KV cache, the execution plane maps each model to appropriate parallelism and kernels, and the control plane coordinates proper P/D ratio and request scheduling.](https://vllm.ai/blog-assets/figures/2026-09-08-vllm-agentx/full-stack-overview.png)
...
![Figure 4: vLLM's hybrid KV cache manager. One page size serves all attention types, and a single block pool is shared between them.](https://vllm.ai/blog-assets/figures/2026-09-08-vllm-agentx/hybrid-kv-cache-manager.png)
...
[Interactive figure](https://vllm.ai/blog-assets/interactive\_pages/dsv4-kv-cache-layout.html)
...
![Figure 6: For Kimi K3, DCP8 achieves lower decode latency than TP8 and scales to higher concurrency.](https://vllm.ai/blog-assets/figures/2026-09-08-vllm-agentx/k3-tp8-vs-dcp8.png)
...
![Figure 7: MLA decode path under DCP4 using symmetric memory. Each step is fused into a single kernel, replacing the NCCL all-gather, staging copy, all-to-all, and unpack steps.](https://vllm.ai/blog-assets/figures/2026-09-08-vllm-agentx/k3-dcp-symmem.gif)
...
![Figure 8: For Kimi K3, wide EP (DEP16) scales better than DCP8 once the per-rank batch size exceeds 3.](https://vllm.ai/blog-assets/figures/2026-09-08-vllm-agentx/k3-dcp8-vs-dep16.png)
...
![Figure 9: Head-of-line blocking in the prefill queue, session view of one rank. Left: without a chunk cap, a long prefill claims the whole budget and the short cached turns wait. Right: with a 512-token cap, short turns join every step and begin decoding sooner.](https://vllm.ai/blog-assets/figures/2026-09-08-vllm-agentx/hol-blocking.gif)
...
![Figure 10: Prefill schedule cadence across a DEP8 group. Left: prefills arrive on different steps and stall the lockstep group repeatedly. Right: with an interval of 4, prefills coalesce onto the cadence steps and the steps in between are decode-only.](https://vllm.ai/blog-assets/figures/2026-09-08-vllm-agentx/prefill-schedule-interval.gif)
...
![Figure 11: Total tokens per $1 under varying P90 interactivities with Kimi K3 running on various hardware. Source: Kimi K3 SemiAnalysis AgentX Dashboard.](https://vllm.ai/blog-assets/figures/2026-09-08-vllm-agentx/k3-agentx-dashboard.png)
...
| Model | GPUs / concurrency | Total tokens per GPU-second (TPGS)[1](#note-tpgs) @ P90 > 50 tok/s | P90 interactivity |
| --- | --- | --- | --- |
...
| Model | GPU TCO/hour | Equivalent Opus 5 cost/hour[2](#note-opus) | Cost advantage |
| --- | --- | --- | --- |
