...
[#reinforcement-learning](https://vllm.ai/blog/tags/reinforcement-learning)[#hardware](https://vllm.ai/blog/tags/hardware)[#ecosystem](https://vllm.ai/blog/tags/ecosystem)[#post-training](https://vllm.ai/blog/tags/post-training)
...
![vime architecture.](https://vllm.ai/blog-assets/figures/2026-07-10-vime-rocm/data-buffer.png)
...
- **Megatron-LM training backend:** vime uses Megatron-LM as the training engine. On ROCm, the Megatron stack builds cleanly using a ROCm-compatible fork and a small patch that guards CUDA fused-kernel initialization on non-CUDA builds. The training loop runs with ROCm-compatible Megatron patches and ROCm-specific launch flags. Gradient accumulation uses the native PyTorch path, which is fully supported on ROCm. Checkpoint conversion from HuggingFace format to Megatron's torch\_dist format runs on a single GPU and completes cleanly, producing a layout that Megatron loads correctly at the start of each training run.
...
> **Note**: On ROCm, use HIP\_VISIBLE\_DEVICES in place of CUDA\_VISIBLE\_DEVICES to select GPUs.
...
![Throughput tending slightly upward with Qwen3-8B model on MI355X.](https://vllm.ai/blog-assets/figures/2026-07-10-vime-rocm/image.png)
...
![Train-rollout logprob absolute difference holding steady around 0.012 with Qwen3-8B model on MI355X.](https://vllm.ai/blog-assets/figures/2026-07-10-vime-rocm/image-1.png)
...
![Raw reward climbing from near 0 to around 0.5~0.6 by step 100 with Qwen3-8B model on MI355X.](https://vllm.ai/blog-assets/figures/2026-07-10-vime-rocm/image-2.png)
