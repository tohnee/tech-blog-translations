...
vLLM-Omni Team and Ant Group SCT Team

[#performance](https://vllm.ai/blog/tags/performance)[#multimodal](https://vllm.ai/blog/tags/multimodal)[#vllm-omni](https://vllm.ai/blog/tags/vllm-omni)
...
![Figure 1: Qwen3-Omni serving in vLLM-Omni is a staged dataflow: Thinker produces text and hidden states, Talker produces codec codes, and Code2Wav reconstructs audio.](https://vllm.ai/blog-assets/figures/2026-07-01-qwen3-omni-optimization/qwen3-omni-serving-flow.svg)
...
![Figure 2: Qwen3-Omni performance comes from optimizing the staged dataflow, stage runtime, and decode hot path together.](https://vllm.ai/blog-assets/figures/2026-07-01-qwen3-omni-optimization/qwen3-omni-optimization-stack.svg)
...
![Figure 3: Each stage captures graphs at a different point. Thinker and Talker decode under vLLM's outer graph; Talker's inner code predictor is torch.compiled instead of given a second graph; Code2Wav uses an inner CUDAGraphDecoderWrapper.](https://vllm.ai/blog-assets/figures/2026-07-01-qwen3-omni-optimization/qwen3-omni-cuda-graph-stages.svg)
...
![Figure 4: Without async chunk, each stage waits for the previous stage's full payload, so first audio tracks full Thinker generation plus Talker prefill. Async chunk overlaps partial handoffs, so Code2Wav starts emitting audio after only a few codec frames.](https://vllm.ai/blog-assets/figures/2026-07-01-qwen3-omni-optimization/qwen3-omni-async-chunk-timeline.svg)
...
![Figure 5: Decode step gap before and after async output. With synchronous payload construction, the GPU sits idle ~2.8 ms between Talker steps; moving payload assembly off the decode path packs steps back-to-back, shrinking the inter-step gap to ~41 µs.](https://vllm.ai/blog-assets/figures/2026-07-01-qwen3-omni-optimization/qwen3-omni-async-output-step-gap.svg)
...
![Figure 6: Async chunk and stage replicas target the speech-generation side of the pipeline, where Talker and Code2Wav can become the bottleneck under concurrent load.](https://vllm.ai/blog-assets/figures/2026-07-01-qwen3-omni-optimization/qwen3-omni-async-replica.svg)
...
![Figure 7: Request throughput (req/s) at c=1 (orange), c=16 (purple), c=32 (green), and c=64 (red). Stage replicas reach 11.7 req/s at c=64 and 6.8 req/s at c=32, up from 2.2 req/s (Batch at c=64).](https://vllm.ai/blog-assets/figures/2026-07-01-qwen3-omni-optimization/qwen3-omni-bench-reqps.svg)
...
![Figure 8: Mean audio RTF at c=1, 16, 32, and 64. Batch sits at or above real-time under load (RTF up to 1.15 at c=64); async output and replicas keep c=32/c=64 RTF at or below ~0.47.](https://vllm.ai/blog-assets/figures/2026-07-01-qwen3-omni-optimization/qwen3-omni-bench-rtf.svg)
...
![Figure 9: Mean audio TTFP in milliseconds (log scale) at c=1, 16, 32, and 64. Async chunk drops c=64 TTFP from ~5884 ms (Batch) to ~655 ms.](https://vllm.ai/blog-assets/figures/2026-07-01-qwen3-omni-optimization/qwen3-omni-bench-ttfp.svg)
