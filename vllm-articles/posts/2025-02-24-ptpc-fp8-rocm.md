...
[#quantization](https://vllm.ai/blog/tags/quantization)[#hardware](https://vllm.ai/blog/tags/hardware)
...
- **$X$**: Input activation tensor ($T\times C_{i}$)
- **$W$**: Weight tensor ($C_{i}\times C_{o}$)
- **$T$**: Token sequence length
- **$C_{i}/C_{o}$**: Input/output channels
- **$\ast$**: Matrix multiplication

**Scaling Factors:**

- **Top (Per-Tensor)**: Single scalars $Δ_{X}[1]$ and $Δ_{W}[1]$ for entire tensors
- **Bottom (PTPC)**: Vector $Δ_{X}[T\times1]$ with one scale per token and $Δ_{W}[1\times C_{o}]$ with one scale per input channel
...
3. **Byte-level metrics** (bits\_per\_byte and byte\_perplexity) show the same pattern of results
