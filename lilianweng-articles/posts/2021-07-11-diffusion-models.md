...
<a name="nice"/>A nice property of the above process is that we can sample $$\mathbf{x}_t$$ at any arbitrary time step $$t$$ in a closed form using [reparameterization trick](https://lilianweng.github.io/posts/2018-08-12-vae/#reparameterization-trick). Let $$\alpha_t = 1 - \beta_t$$ and $$\bar{\alpha}_t = \prod_{i=1}^T \alpha_i$$:
